...
if __name__ == "__main__":
    ...
    for i in range(frameSize):
        for j in range(frameSize):
            noise[i,j] = PNFactory(i*10.0/frameSize,j*10.0/frameSize)
    cv2.imshow("rr",np.uint8(noise*100+100))
    cv2.waitKey(1000)
    ...
    PNFactory = PerlinNoiseFactory(1)
    noise=np.zeros(512)
    frameSize = 512
    for i in range(frameSize):
        noise[i] = PNFactory(i*10.0/frameSize)
    frame = np.uint8(noise*100+100).reshape((1,frameSize))
    frame = cv2.resize(frame,(frameSize,100),interpolation=cv2.INTER_NEAREST)
    cv2.imshow("rr",frame)
    cv2.waitKey(1000)
